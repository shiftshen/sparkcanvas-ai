...
- `backend/`：Express + TypeScript API，提供演示登录、品牌档案、模板库、画布历史、品牌注入工作流和 VDAMO 图片 API 调用。未配置有效密钥时会明确降级到内置品牌素材，便于离线开发和回滚。
...
图片生成由后端直接调用 VDAMO OpenAI-compatible `/v1/images/generations`，不在前端直接请求图片 API。默认图片模型为 `gpt-image-2`，可切换 `gpt-image-1.5` 和 `gpt-image-1`；旧的本地 image skill、`nano_banana_2` 和 cliproxyapi 图片路由不再进入生产选择器。文本默认走 VDAMO `/v1/chat/completions` 的 `gpt-5.4-mini`，也可通过 Gemini 分组密钥使用 Gemini 文本模型。视频仍走 yijiarj `/v1/videos`，参考图必须按模型能力表传公网 `input_reference` 链接和 `size`，不能传本地路径或旧的 `image_url/aspect_ratio`。
...
export VDAMO_BASE_URL='https://api.vdamo.com/v1'
export VDAMO_OPENAI_API_KEY='YOUR_VDAMO_OPENAI_GROUP_KEY'
export VDAMO_GEMINI_API_KEY='YOUR_VDAMO_GEMINI_GROUP_KEY'
...
export TEXT_GEN_BASE_URL='https://api.vdamo.com/v1'
export TEXT_GEN_MODEL='gpt-5.4-mini'
npm run dev
```
...
- `gpt-image-2`：默认图片模型，走 VDAMO OpenAI-compatible `/v1/images/generations`，已通过真实 PNG 出图测试。视频目标时先生成分镜/首帧参考图；海报/广告图目标时才生成最终广告图片。
- `gpt-image-1.5` / `gpt-image-1`：VDAMO OpenAI 分组图片候选，已通过真实 PNG 出图测试，可作为回退或对比模型。
- Gemini 图片模型：VDAMO `/v1/models` 可列出 `gemini-2.5-flash-image` 和 `gemini-3.1-flash-image`，但当前探测没有得到可用图片输出或返回账号不可用，暂不开放生产选择器，只在诊断里保留。
...
- `/docs/`：包含所有的核心设计文档。
  ...
  - `SparkCanvas_Backend.md`：后端 Express + 本地 JSON 持久化 + AI API 调用方案。
