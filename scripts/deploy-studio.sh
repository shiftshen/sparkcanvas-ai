...
ssh "$REMOTE" "bash -lc 'cd \"$DEPLOY_PATH\" || exit 1; set -a; [[ -f .env.production ]] && source .env.production; set +a; u=\${STUDIO_BASIC_AUTH_USER:-}; p=\${STUDIO_BASIC_AUTH_PASSWORD:-}; for i in \$(seq 1 40); do noauth=\$(curl -sS -o /dev/null -w \"%{http_code}\" http://127.0.0.1:23090/ || true); authed=\$(curl -sS -o /dev/null -w \"%{http_code}\" -u \"\${u}:\${p}\" http://127.0.0.1:23090/ || true); api=\$(curl -sS -o /dev/null -w \"%{http_code}\" -u \"\${u}:\${p}\" http://127.0.0.1:23090/workgraph-os/workspace || true); if [[ \"\$noauth\" == \"401\" && \"\$authed\" == \"200\" && \"\$api\" == \"200\" ]]; then echo \"studio gateway healthy (no-auth=401 authed=200 api=200)\"; exit 0; fi; sleep 3; done; echo \"studio gateway health check failed (no-auth=\$noauth authed=\$authed api=\$api)\" >&2; exit 1'"
