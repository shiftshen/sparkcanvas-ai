...
ssh "$REMOTE" "bash -lc 'set -u; for i in \$(seq 1 40); do auth_status=\$(curl -sS -o /dev/null -w \"%{http_code}\" http://127.0.0.1:23080/api/auth/config || true); ai_status=\$(curl -sS -o /dev/null -w \"%{http_code}\" -H \"Authorization: Bearer demo-token\" http://127.0.0.1:23080/api/ai/status || true); if [[ \"\$auth_status\" == \"200\" && \"\$ai_status\" == \"200\" ]]; then exit 0; fi; sleep 3; done; echo \"marketing health check timed out\" >&2; exit 1'"
