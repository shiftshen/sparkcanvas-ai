...
assert(serverSource.includes("startWorkGraphOutputWatcher") && serverSource.includes("workGraphOsWatchDirs") && serverSource.includes("fsWatch(dir") && serverSource.includes("registerWatchedOutputFile"), "backend should watch the output dir(s) and register new media as assets");
...
assert(serverSource.includes("workGraphOsWatchDirs") && serverSource.includes("WGOS_PIWEB_CWD") && serverSource.includes("!== path.resolve(projectRoot)"), "output watcher should also watch the explicit pi-web cwd but never the repo root");
